...
class SportcalenderController extends AbstractController
{
    ...
    public function editAction(Request $request, $id): Response
    {
        ...
        $sport->setCategory($sport->getCategory());
        $sport->setDescription($sport->getDescription());
        $sport->setPriority($sport->getPriority());
        $sport->setDueDate($sport->getDueDate());
        $sport->setCreateDate($now); */

        $form = $this->createFormBuilder($sport)->add('name', TextType::class, array('attr' => array('class' => 'form-control', 'style' => 'margin-botton:15px')))
            ->add('category', TextType::class, array('attr' => array('class' => 'form-control', 'style' => 'margin-bottom:15px')))
            ...
            ->add('save', SubmitType::class, array('label' => 'Update sport', 'attr' => array('class' => 'btn-primary', 'style' => 'margin-botton:15px')))
            ->getForm();
        $form->handleRequest($request);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $now = new \DateTime('now');
            $em = $this->getDoctrine()->getManager();
            $sport = $em->getRepository('App:Sport')->find($id);
            $sport->setName($name);
            $sport->setCategory($category);
            $sport->setDescription($description);
            $sport->setPriority($priority);
        ...
        return $this->render('sportcalender/edit.html.twig', array('sport' => $sport, 'form' => $form->createView()));
    }

    ################## DELETE CONTROLLER ###########################

    /**
     * @Route("/delete/{id}", name="sport_delete")
     */
